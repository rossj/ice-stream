 ...
 * @param {string|function(string)} mixed
 */
istream.dropUntilChunk = function (mixed, emitMatch) {
	if ( !_.isString(mixed) && !_.isFunction(mixed) )
		throw new Error('Invalid parameter');

	if ( _.isString(mixed) && !mixed.length )
		throw new Error('Invalid parameter');

	var match = _.isFunction(mixed) ? mixed : function(chunk) {
		return chunk === mixed;
	};
