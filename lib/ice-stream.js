...
istream.count = function (stream, cb) {
	...
	stream.on('data', function (chunk) {
		...
		if ( _.isString(chunk) ) {
			...
			chars += chunk.toString().length;
			bytes += chunk.length;
		}
	});

	stream.on('end', function () {
		cb(chunks, chars, bytes);
	});
};

/**
 * When the stream finishes, this returns the string length count to the callback
 * @param stream
 * @param cb
 */
istream.chars = function (stream, cb) {
	var count = 0;
	stream.on('data', function (chunk) {
		if ( _.isString(chunk) ) {
			count += chunk.length;
...
/**
 * When the stream finishes, this returns the byte count to the callback
 * @param stream
 * @param cb
 */
istream.bytes = function (stream, cb) {
	var count = 0;
	stream.on('data', function (chunk) {
		if ( _.isString(chunk) ) {
...
function createChainableMethodFromStatic(func) {
	return function () {
...
[
	...
	istream.prototype[func] = createChainableMethodFromStatic(func);
});

// Add new-style streams
/**
 *
 * @param {string} name
 * @param {Function} Stream a stream constructor
 */
var add = function(name, Stream) {
	istream[name] = Stream;
	istream.prototype[name] = createChainableMethodFromStatic(name);
};

add('base64decode', require('base64-stream').Decode);
add('base64encode', require('base64-stream').Encode);
add('filter', require('./filter.js'));
add('reject', require('./reject.js'));

// Add the stream consumer functions to the prototype
['out', 'each', 'count', 'chars', 'bytes'].forEach(function (func) {
